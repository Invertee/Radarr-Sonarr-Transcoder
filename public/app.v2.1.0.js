...
async function loadProfiles() {
  ...
  const options = profileOptions('medium');
  elements.manualProfile.innerHTML = options;
  elements.episodeProfile.innerHTML = options;
  elements.movieProfile.innerHTML = options;
}
...
function markMediaQueued(item, button, result) {
  item.queueStatus = result?.job?.status || 'queued';
  const row = button.closest('tr');
  const stateCell = row?.querySelector('.media-state-cell');
  if (stateCell) {
    stateCell.innerHTML = mediaState(item);
  }
  button.textContent = item.queueStatus;
  button.dataset.persistDisabled = 'true';
...
async function handleAction(event) {
  ...
  try {
    ...
    if (action === 'queue-up') {
      ...
      const index = Number(button.dataset.index);
      const result = await queueMedia(state.sonarrFiles[index], elements.episodeProfile.value);
      markMediaQueued(state.sonarrFiles[index], button, result);
      ...
      const index = Number(button.dataset.index);
      const result = await queueMedia(state.movies[index], elements.movieProfile.value);
      markMediaQueued(state.movies[index], button, result);
    }
  } catch (error) {
    showToast(error.message, true);
  } finally {
    if (button.isConnected && button.dataset.persistDisabled !== 'true') {
...
function cacheElements() {
  for (const id of [
    ...
    'episodeSearch', 'episodeProfile', 'episodesBody', 'refreshMoviesButton', 'movieSearch',
    'movieProfile', 'moviesBody', 'refreshHistoryButton', 'historySearch', 'historyBody',
